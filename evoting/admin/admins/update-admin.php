...
if (isset($_POST['submit'])) {
    if (empty($_POST['password'])) {
        redirects("update-admin.php?adminID=$id", "password field is empty");
    } else {
        $password = md5($_POST['password']);

        $update = $conn->prepare("UPDATE admins SET `password`=:password");
        $update->bindParam(':password', $password);

        if ($update->execute()) {
            redirect("admins.php", "password changed successfully");
        }
    }
}
?>
                ...
                <div class="col">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="POST" action="update-admin.php?adminID=<?php echo $details->adminID; ?>" enctype="multipart/form-data">
                                <!-- Email input -->
